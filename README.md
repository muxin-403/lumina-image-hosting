...
│       ├── deleteKey.js         上传删除凭证（HMAC 签发 / 常数时间校验）
...
一次拖入 20 张图时，逐张串行上传的瓶颈往往不是带宽而是**往返延迟**：每张都要等
「请求 → 服务端处理 → 响应」整轮走完，才轮到下一张。上传页改为 worker 池并发后，
同时在途张数由 `client_max_concurrency` 决定（默认 3，管理台「客户端上传行为」可热改）：

- **槽位隔离**：每张图按批次下标占用独立槽位，结果、进度、失败原因都与原文件
  一一对应 —— 不依赖完成顺序，也不需要加锁；结果列表最终按选择顺序合并展示。
- **逐张进度**：每条任务卡片各自持有一条进度条，只反映自己那条请求的已传字节比例，
  并发时互不干扰，也不存在「某一张卡住时整体进度停滞」的观感（详见 4.6）。
- **失败隔离**：单张失败只影响自己（独立 toast 提示文件名与原因），其余照常完成，
  不存在「第 5 张挂了、后面全白等」的情况。
- **上限取 6 的原因**：HTTP/1.1 下浏览器对同源并发连接本就限制在 6 条左右，
  配置更高只会让请求在浏览器侧排队，拿不到额外收益。
- **命令行同样支持**：`node examples/upload.js upload *.png --concurrency 3`
  （或 `LUMINA_CONCURRENCY=3`，上限 10）。

### 4.6 上传列表：逐张进度与单张删除

批量上传时「一条总进度条」只能回答「整体还剩多少」，回答不了「是哪一张卡住了」。
上传页因此把**上传任务列表与结果列表合并成同一个列表**：

- **提交即上屏**：文件一进入批次，就先为每张图建立一条卡片（本地 `objectURL` 缩略图
  + 文件名 / 体积 + 自己的进度条 + 状态徽标）；上传完成后同一张卡片就地升级为带
  5 种引用格式的结果卡片。全程只有一个列表，不会重复展示同一张图。
- **状态机**：`queued →（可选 converting）→ uploading → done | failed`。
  排队与浏览器内转码阶段拿不到字节级进度，进度条走不确定态动画，不假装一个百分比。
- **任意状态都能单独删除**：
  - 上传中删除 → `xhr.abort()` 中断在途请求；若该张还在排队，worker 领取到它时
    会直接跳过；同时释放本地预览 URL。
  - 已完成删除 → 连同**服务端原图、缩略图与数据库记录**一起清理。
  - 两者都只影响被删的那一条，其余任务照常跑完。
- **删除凭证 `delete_key`**：游客没有管理员 Token，却要能撤销自己刚上传的图。
  上传响应因此为**本次新建**的记录签发一枚 HMAC 凭证
  （`HMAC-SHA256(sessionSecret, "lumina:delete:v1:<id>:<sha256>")`，服务端用
  `timingSafeEqual` 比对），前端删除时带上即可，无需登录；凭证不可伪造，且只对签发
  时的那一条记录有效。秒传命中的记录**不下发**凭证，避免后上传者删掉先上传者的文件。
- **「删除」与「清空列表」的区别**：卡片上的删除会清理服务端资源；列表头的清空
  只清空本页展示，服务器上的图片不受影响。
...
| `CLIENT_MAX_CONCURRENCY` | `3` | 上传页批量上传的最大并发数（1–6，管理台可热改） |
...
用 `npm test` 一次性跑通全部三套测试，共 **179 项断言，0 失败**
（Windows 11 + Node v22.22.2，服务监听 3200 端口）：

| 测试套件 | 命令 | 断言数 | 结果 |
| --- | --- | --- | --- |
| 端到端接口 | `npm run test:api` | 62 | ✅ 全通过 |
| WebDAV 驱动 | `npm run test:webdav` | 32 | ✅ 全通过 |
| 前端运行时 | `npm run test:ui` | 85 | ✅ 全通过 |
...
- **上传凭证删除**：新建记录的上传响应下发 `delete_key`；不带凭证或伪造凭证删除仍 401；
  携带合法凭证可删除，且直链与元数据同步 404；秒传命中的记录**不下发**凭证
...
- `src/`、`public/js/`、`scripts/`、`examples/` 下全部 35 个 JS 文件通过 `node --check` 语法校验
