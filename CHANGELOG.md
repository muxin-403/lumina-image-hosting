...
- **全站界面可读性优化（不改功能与业务逻辑）**：按「问题清单 → 逐项落地」完成一轮文案与视觉层次整理。
  - 上传页：hero 与拖拽区不再重复两套上传说明；限制提示改为「单张 ≤ X，一次最多 N 张」；批量操作收敛为「复制全部链接 + 更多 ▾ 下拉（Markdown / BBCode / 导出 .txt）」，「清空列表（不删图片）」把关键前提前置到可见文案；没有任何完成项时点批量按钮不再静默无反应，改为给出提示并弱化按钮（`aria-disabled`）；列表头在途计数并入「N 张上传中 · 已完成 M 张」；失败卡片不再显示已无意义的进度条，直接突出「失败原因」；「秒传 / 矢量 / 浏览器内转码」等术语改为用户视角表述（如「与已有图片相同，已复用原文件」「矢量图（SVG）」「正在转换格式」）；页脚去技术栈自述。
  - 管理台：改写四处晦涩说明（游客上限「核心开关」、服务端「负优化自动回退」、「前端不再展示这些开关」、并发数 HTTP/1.1 细节）；存储驱动选项中文化（value 不变）；「直链说明」由表单字段改为提示块；设置页新增右下角常驻保存按钮（`form` 属性提交同一表单，改到下方卡片无需滚回顶部）；图片管理工具栏拆两行并为每个筛选下拉加可见标签；表格单行「删除」降为悬停变红（批量删除仍为红色实心）；统计卡存储方式中文化、「格式分布」超长截断并带悬浮提示、今日无上传显示「今日暂无上传」；「程序化接入」改名「API 访问令牌」；初始密码告警附「去修改 →」直达按钮。
  - 404 页：文案区分「被删除」与「链接不完整」两种可能。
  - 术语统一：面向用户的文案统一「链接 / 图片 / 访客」用词，toast 拆掉长括号补语。
- 进度更新改为**只重绘该张卡片的进度条**（整数百分比变化时才写 DOM），不再整表重绘，避免打断用户正在进行的复制 / 切换引用格式操作。
- 卡片定位由数组下标改为任务 `uid`：删除任意一条后，其余卡片的索引与交互不再错位。
- 版本号对齐：`package.json` / `package-lock.json` 由 `1.2.0` 提升至 `1.3.0`，与本文档最新版本一致。
...
- `examples/ui-check.js`：3 处 WebP 徽标断言随界面文案更新（「客户端转 WebP」→「已转 WebP 省空间」）；`README.md` 4.7 批量复制按钮描述同步为新界面文案。
...
| 版本 | 发布日期 | 主要变更 | 提交区间 |
| --- | --- | --- | --- |
| [1.3.0] | 2026-09-19 | 上传列表逐张进度与单张删除（含删除凭证 `delete_key`）、全站界面可读性优化 | `28e4a68`...`HEAD` |
...
[1.3.0]: https://github.com/muxin-403/lumina-image-hosting/compare/a5b6161...HEAD
